import{L as e,T as t,Y as n,_ as r,b as i,g as a,gt as o,mt as s,v as c,x as l}from"./modules/shiki-BiiZU2CK.js";import{n as u,t as d}from"./slidev/context-CitRXyF7.js";import{t as f}from"./DeckImage-BTSuGCwJ.js";import{t as p}from"./slidev/default-FzZMvvLY.js";var m={class:`oficina-canvas amazonia-slide`},h={__name:`oficina-unb.md__slidev_52`,setup(h){let{$slidev:g,$nav:_,$clicksContext:v,$clicks:y,$page:b,$renderContext:x,$frontmatter:S}=u();return v.setup(),(u,h)=>{let g=f;return e(),r(p,o(t(s(d)(s(S),51))),{default:n(()=>[a(`div`,m,[h[0]||=a(`h1`,{class:`slide-title pos-title-left title-tight`},`Mudança de função na Amazônia`,-1),h[1]||=a(`section`,{class:`amazonia-block finbow-left-block`},[a(`p`,null,[i(`No século XVIII, o termo `),a(`em`,null,`Língua Geral`),i(` passa a apontar menos para um bloco geográfico de falantes nativos e mais para uma língua franca supraétnica.`)]),a(`blockquote`,{class:`quote-text compact`},` “a supra-ethnic lingua franca used as an auxiliary language by many peoples” `),a(`div`,{class:`amazonia-keypoint`},[a(`p`,{class:`point-label`},`Ponto-chave`),a(`p`,null,`A mudança do nome e da função acompanha a incorporação forçada de muitos povos não-tupi e não-tupi-guarani ao sistema colonial amazônico.`)])],-1),l(g,{class:`manuscript pos-anchieta-main`,image:`14_finbow2025_p15_bluteau_tapuia_lg.png`,alt:`Finbow 2025, p. 15, Bluteau e a Língua Geral como língua franca supraétnica falada por muitos povos`}),h[2]||=a(`p`,{class:`source finbow-source`},`Finbow, Thomas. “A sociophilological account of the formation and evolution of the term Língua Geral, with emphasis on Amazonia.” SciELO Preprints, 2025, p. 15.`,-1)]),c(`
Imagem citada: 14_finbow2025_p15_bluteau_tapuia_lg.png.
Fonte visual: Finbow, Thomas. “A sociophilological account of the formation and evolution of the term Língua Geral, with emphasis on Amazonia.” SciELO Preprints, 2025, p. 15.

A imagem mostra Finbow explicando Bluteau: no século XVIII, a Língua Geral já não é pensada apenas como uma língua ou contínuo regional de falantes nativos, mas como língua franca supraétnica usada como língua auxiliar por muitos povos. Essa formulação ajuda a passar da questão “LGP/LGA como variedades regionais” para a questão amazônica: a função social da língua mudou.
`),c(` ---
class: oficina-unb
---

<div class="oficina-canvas amazonia-slide">
  <h1 class="slide-title pos-title-left title-tight">Duas normas dentro da Língua Geral</h1>

  <section class="amazonia-block finbow-left-block">
<p>Ao ler João Daniel, Finbow observa que tanto a norma escrita quanto a fala cotidiana ainda são chamadas de <em>Língua Geral</em>.</p>
<blockquote class="quote-text compact">
  “both the H and the L varieties are referred to as ‘LG’ by Daniel”
</blockquote>
<div class="amazonia-keypoint">
  <p class="point-label">Leitura possível</p>
  <p>Não uma ruptura absoluta entre línguas, mas uma tensão entre norma catequética antiga e vernáculo amazônico cotidiano.</p>
</div>
  </section>

  <DeckImage
    class="manuscript pos-anchieta-main"
    image="05_finbow2022_p84_85_diglossia_h_l_lg.png"
    alt="Finbow 2022, pp. 84-85, leitura diglóssica de João Daniel sobre norma escrita e vernáculo cotidiano da Língua Geral"
  />
  <p class="source finbow-source">Finbow, Thomas. “The Nature and Emergence of the Língua Geral Amazônica according to Mufwene’s Language Ecology Model.” <em>Revista do GEL</em>, 19(2), 2022, p. 84–85.</p>
</div> `)]),_:1},16)}}};export{h as default};